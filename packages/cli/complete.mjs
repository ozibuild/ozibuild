  ...
  try {
    ...
    for (let subdir = dir.readSync(); subdir; subdir = dir.readSync()) {
      ...
      if (subdir.isDirectory()) {
        dirnames.push(join(dirPath, subdir.name));
      }
    }
    dir.close();
  } catch (error) {
  ...
  try {
    ...
    for (let file = dir.readSync(); file; file = dir.readSync()) {
      if (file.isFile() && hasJavascriptExt(file.name)) {
        scripts.push(join(dirPath, file.name));
      }
    }
    dir.close();
  } catch (error) {
...
function completeDirs(target) {
  ...
  return dirs.filter((dir) => dir.startsWith(target));
}
...
function completeScripts(target) {
  ...
  return scripts.filter((script) => script.startsWith(target));
}
...
export async function complete(args) {
  ...
  if (compCWord === 1) {
    completions.push(
      ...["config", "make", "watch"].filter((cmd) => cmd.startsWith(word)),
    );
  }
