 ...
 * @param params.outfile Indicates that the build command generates an output file.
 * In this case a successful run of the command must produce the ouput.
 * @param params.stdoutfile Indicate to produce outfile from stdout.
...
export async function buildCmd(ctx: SourceDirContext,
  ...
  try {
    ...
    if (params.stdoutfile && params.outfile) {
      writeFileSync(params.outfile, stdout);
  ...
  return params.outdir || params.outfile || params.label || bin;
}
