...
export async function build(
  ...
  const prefix =
    nullOrRelativePath(ctx, out.file) ||
    nullOrRelativePath(ctx, out.dir) ||
    out.label ||
    bin;
  prefixInfo(
    prefix,
    `\x1b[35m${bin}\x1b[0m ${args.join(" ")}    \x1b[90m#cwd: ${cwd}\x1b[0m`,
  );
  ...
  const outfile = out.file ? ctx.outputFile(out.file) : undefined;
  try {
  ...
  if (outfile && !existsSync(outfile)) {
    throw Error(
      `Output ${out.file} was not produced by the commad [${bin} ${args.join(" ")}]`,
    );
  }
  const outdir = out.dir ? ctx.outputFile(out.dir) : undefined;
  if (outdir && !existsSync(outdir)) {
    throw Error(
      `Output ${out.dir} was not produced by the commad [${bin} ${args.join(" ")}]`,
    );
  }
